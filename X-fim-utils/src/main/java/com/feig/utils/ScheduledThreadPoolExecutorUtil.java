...
public class ScheduledThreadPoolExecutorUtil {
    ...
    public void execute(Runnable task) {
        ...
        LOG.debugInfo("当前任务数为：{}", startCount);
        ...
        executor.execute(() -> {
            try {
                ...
                LOG.debugInfo("当前任务已完成，用时：{}ms， 剩余任务数：{}",System.currentTimeMillis() - startTime, endCount );
            }
        });
    }
    ...
    public void schedule(Runnable task, long delay, TimeUnit timeUnit) {
        ...
        LOG.debugInfo("当前定时任务数为：{}", startCount);
        final long startTime = System.currentTimeMillis();
        executor.schedule(() -> {
            try {
                task.run();
                ...
                LOG.debugInfo("当前定时任务已完成，用时：{}ms， 剩余定时任务数：{}",System.currentTimeMillis() - startTime, endCount );
            }
        },delay,timeUnit);
    }
